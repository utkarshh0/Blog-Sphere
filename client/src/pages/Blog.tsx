...
const Blog: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBlogs = async () => {
            try {
                const response = await axios.get('http://localhost:3000/api/v1/user/blog', {
                    headers: {
    ...
    return (
        <>
            <div className="w-screen h-screen overflow-x-hidden p-16 md:p-16 lg:p-24">
                <div onClick={() => setAddBlog(true)} className="text-4xl p-3 border-b"><button className="cursor-pointer hover:opacity-50"><IoIosAddCircleOutline /></button></div>
                {blogs.map(blog => (
                    
                    <BlogCard key={blog._id} blog={blog} onClick={() => {setBlogPost(true); setCurrentBlog(blog)}}/>
                ))}
            </div>
